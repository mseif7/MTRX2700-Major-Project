...
void writeStringAndIntLCD(unsigned char * message, int number ){
  COMWRT4(0x01);           //move the curose to the top left corrner of the LCD
  writeStringLCD(message); //print message to LCD
  writeIntLCD(number);    //write number straight after the string
  COMWRT4(0x80);           //set start posistion, home position
  MSDelay(1);              //delay for a short period of time
}


void stringIntStringLCD(unsigned char * message1, int number, unsigned char * message2){
  COMWRT4(0x01);           //move the curose to the top left corrner of the LCD
  writeStringLCD(message1);   //print message to LCD
  writeIntLCD(number);      //print number to LCD
...
void writeTwoStringsSndTwoSIn(unsigned char * message1, int number1, unsigned char * message2, int number2){
  ...
  writeStringLCD(message1);   //print message to LCD
  writeIntLCD(number1);      //print number to LCD
  
  COMWRT4(0xC0);              //set start posistion, home position
  MSDelay(1);                 //delay for a short period of time
  
  writeStringLCD(message2);   //print message to LCD
  writeIntLCD(number2);      //print number to LCD
...
void activateDisplayLCD(unsigned int lcd_flag, int itemQty, int fullQty, char *fullMsg, char *emptyMsg, char *refMsg1, char *refMsg2, char *errorMsg) {
  
  int refill_qty = fullQty - itemQty;
  
  if (lcd_flag == 1) {
    
    if (itemQty == fullQty) {
	    writeStringLCD(fullMsg);
	
	  }	else if (itemQty == 0) {
	    writeStringLCD(emptyMsg);

	  } else if (itemQty < fullQty){
	    stringIntStringLCD(refMsg1, refill_qty, refMsg2);
	  } else if (itemQty > fullQty) {
	    writeStringLCD(errorMsg);
	  }
